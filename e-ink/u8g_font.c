...
void u8g2_u8gFont(u8g2_t *u8g2) {
	...
	u8g2->scale							= 1;
	u8g2->font_decode.fg_color			= MODE_CLEAR;
	u8g2->font_decode.bg_color 			= MODE_BLACK;
...
static u8g2_uint_t u8g2_DrawGlyph(u8g2_t *u8g2, u8g2_uint_t x, u8g2_uint_t y, uint16_t encoding) {
	if (u8g2->font_decode.buff_width > 0) {
		y += u8g2->font_info.max_char_height + u8g2->font_ref_descent;
	} else {
...
u8g2_uint_t u8g2_allocateBitmap(u8g2_t *u8g2, uint8_t** buff, const char *str) {
	uint16_t width	= u8g2_GetStrWidth(u8g2, str);
...
static void u8g2_font_decode_len(u8g2_t *u8g2, uint8_t len, uint8_t is_foreground) {
	...
	for (;;) {
		...
		if (decode->buff_width > 0) {		// Decode to the buffer
			if (is_foreground && y < u8g2->font_info.max_char_height) {
			...
			for (uint8_t l = 0; l < scale; ++l) {
				if (is_foreground) {
					EPD_DrawHLine(x,  y+l,  current * scale, decode->fg_color);
				} else if (decode->is_transparent == 0) {
					EPD_DrawHLine(x,  y+l,  current * scale, decode->bg_color);
				}
			}
		}
